...
We evaluate R4RR on three complementary benchmarks of shortcut learning and distribution shift: \emph{DecoyMNIST}~\cite{erion2021improving}, \emph{Waterbirds} \cite{sagawa2019distributionally}, and \emph{RedMeat}, derived from Food-101~\cite{bossard2014food}. Across all three datasets, \textbf{R4RR} consistently outperforms language-guided attention baseline, \emph{GALS}~\cite{petryk2022guiding}, attention-module baseline, \emph{ABN}~\cite{fukui2019attention}, robust-training baseline, \emph{AFR}~\cite{qiu2023simple} and standard empirical risk minimization (ERM) baselines. The gains are especially pronounced when training data provides little or no counter-bias evidence, as in the fully biased Waterbirds setting. We further show through Pointing Game~\cite{zhang2018top} that R4RR shifts model evidence toward task-relevant object regions.
...
\item We demonstrate that R4RR is architecture-general: it applies to ResNet, MobileNet, and ViT students through architecture-specific spatial evidence maps.
...
Attention Branch Network (ABN)~\cite{fukui2019attention} augments a CNN with an attention branch whose output is used both for visualization and classification. Automatic Feature Reweighting (AFR)~\cite{qiu2023simple} improves group robustness by retraining the final layer of an ERM model with weights derived from the model's own errors, without requiring group labels. These approaches are complementary to R4RR. 
...
R4RR requires a spatial evidence map from the student classifier for the ground-truth class. The construction of this map depends on the student architecture. For CNN, we compute student evidence maps using Class Activation Mapping (CAM) \cite{zhou2016learning} at the spatial resolution of the final convolutional block.
Unless otherwise noted, we use standard CAM~\cite{zhou2016learning} for all ResNet\cite{he2016deep} models; for DecoyMNIST~\cite{erion2021improving} we use Grad-CAM \cite{selvaraju2017grad} (on the second convolutional layer) to match the LeNet-style architecture and prior protocol \cite{rieger2020interpretations}. We discuss Vision Transformer specific student evidence in Section~\ref{sec:ViTstudent}.
...
\begin{equation}
\tilde{A}^{\mathrm{cam}}_\theta(x,c)[u,v] = \sum_{d=1}^{D} \mathbf{W}_{c,d}\,F_\theta(x)[d,u,v],
{1,\dots,W_{\mathrm{s}}\}.
\end{equation}
such that, 
\begin{equation}\notag
\qquad (u,v)\in\{1,\dots,H_{\mathrm{s}}\}\times\{1,\dots,W_{\mathrm{s}}\}.
\end{equation}
...
We add an alignment between the teacher distribution to the student distribution using KL divergence in the teacher-to-student direction:
\begin{equation}
...
where
\begin{equation}
\mathrm{KL}(P|Q)=\sum_{u,v}P[u,v]\log\frac{P[u,v]}{Q[u,v]+\epsilon}.
\end{equation}
...
\subsection{Classify-then-Align Curriculm}
...
We select checkpoints using validation performance after the alignment phase begins. This ensures that the selected model reflects the effect of evidence alignment rather than only the cross-entropy warmup. %Algorithm \ref{alg:r4rr} shows our approach. 
...
\textbf{RedMeat}~\cite{petryk2022guiding}. For Waterbirds, we evaluate two settings: Waterbirds$_{95}$ where a small fraction of training samples counter the dominant background-label correlation, and Waterbirds$_{100}$ where the label and background are perfectly correlated during training, making this setting challenging. Full dataset details are provided in Appendix~\ref{datasetdetails}, and group/class counts are reported in Appendix~\ref{sec:supp_dataset_splits}.
...
\begin{groupplot}[
    ...
    ymax=100,
    ylabel={Accuracy (\%)},
    symbolic x coords={Vanilla,CLIP-Z,CLIP-LR,Upweight,ABN,GALS,AFR,R4RR},
    xtick=data,
...
We compare standard ERM (\textbf{Vanilla}), class-balanced training (\textbf{Upweight}), zero-shot CLIP (\textbf{CLIP-Z}), linear probing on CLIP features (\textbf{CLIP-LR}), ABN, GALS, AFR, and our method (\textbf{R4RR}).
Each panel reports mean group accuracy and worst-group accuracy. Higher is better.
%R4RR is especially effective in the fully biased Waterbirds$_{100}$ setting, where the training data contains no counter-bias examples.
}
...
\paragraph{Baselines.}
We compare R4RR against baselines covering standard empirical risk minimization, class balancing, VLM classifiers, language-guided attention supervision, architectural attention, and reweighting-based robustness.

\textbf{Vanilla} trains the student using standard cross-entropy. \textbf{Upweight}~\cite{petryk2022guiding} uses class-balanced cross-entropy with weights inversely proportional to class frequency. \textbf{CLIP-${Zero}$} is a zero-shot CLIP classifier using text prompts for the target classes, while \textbf{CLIP-${LR}$} trains a linear classifier on frozen CLIP image features~\cite{radford2021learning}. \textbf{GALS}~\cite{petryk2022guiding} uses language-grounded maps to guide a classifier through explanation regularization. \textbf{ABN}~\cite{fukui2019attention} augments a CNN with an attention branch trained jointly with the classifier. \textbf{AFR}~\cite{qiu2023simple} starts from an ERM-trained model and retrains the final layer with an automatically weighted loss that emphasizes samples the ERM model predicts poorly.

\paragraph{Metrics.}
Following group-robustness evaluation~\cite{sagawa2019distributionally}, we report two group-level metrics: (i) \emph{average group accuracy} (\textbf{Mean$_{Group}$}), which assigns equal weight to each group regardless of frequency, and (ii) \emph{worst-group accuracy}, defined as the minimum accuracy over the four groups (\textbf{Worst$_{Group}$}). Worst-group accuracy is particularly informative because it reflects performance on the minority and bias-conflicting groups (e.g. \emph{waterbird-on-land} in Waterbirds dataset), which degrade most severely when the model relies on spurious background cues. For Waterbirds, groups correspond to the four combinations of bird type and background. For DecoyMNIST, groups correspond to digit classes. For RedMeat, groups correspond to the five target classes.

\paragraph{Student details.}
Our main experiments use the same student architectures as prior work for each benchmark. For Waterbirds and RedMeat, we train an ImageNet-pretrained ResNet-50~\cite{he2016deep}. For DecoyMNIST, we train a LeNet-style CNN~\cite{lecun1998gradient}. To test architecture generality, we additionally evaluate R4RR with MobileNet~\cite{howard2017mobilenets} and ViT-B/16 students in Section \ref{sec:archgeneralize}. We provide model details in Appendix~\ref{appendix:model_details}. 
...
For each dataset and method (unless noted otherwise), we select hyperparameters using automated validation-based tuning.
Specifically, we evaluate 50 Optuna~\cite{akiba2019optuna} trials per \emph{(dataset, method)} configuration and choose the single best trial by validation performance. We provide hyperparameter optimization details in Appendix~\ref{sec:hparam_opt} and release the validation-selected hyperparameters for all methods, variants, and ablations in our codebase\textcolor{red}{we have to add an anonym repo}.


\subsection{Results}\label{sec:results}

Figure~\ref{fig:main_results} shows the comparison between different methods.

\paragraph{DecoyMNIST.} Standard ERM (\textbf{Vanilla}) and class reweighting (\textbf{Upweight}) collapse completely on the bias-conflicting groups, indicating near-total reliance on the decoy cue. Zero-shot CLIP (\textbf{CLIP$-{Z}$}) shows the same failure mode on worst-group accuracy despite improved average performance. In contrast, our method \textbf{(R4RR)} achieves the highest per-group and worst-group accuracy, substantially outperforming all baselines we compare against. Due to page constraints, we provide qualitative examples in the Appendix~\ref{appendix:qualisamples}.
...
\paragraph{Waterbirds.} In Waterbirds$_{95}$ a small number of counter-bias examples are available during training, allowing reweighting-based methods such as AFR to perform strongly. R4RR remains competitive in this setting and substantially improves over ERM. The advantage of R4RR is most pronounced in Waterbirds$_{100}$, where the training data contains no counter-bias examples. In this fully biased setting, ERM, class balancing, ABN, AFR, and direct VLM classifiers all remain vulnerable to the background shortcut, while R4RR achieves the strongest mean group and worst-group accuracy.

These results also highlight that when the training set is fully biased, the data alone provides little evidence that the background shortcut is unreliable. R4RR addresses this case by injecting a spatial prior from the VLM teacher, guiding the student toward bird evidence rather than background evidence. Due to page constraints, we provide qualitative examples in the Appendix~\ref{appendix:qualisamples}.

\paragraph{RedMeat.} R4RR improves worst-group accuracy over ERM, class balancing, GALS, ABN, AFR, and direct VLM classifiers. CLIP$_{LR}$ obtains the highest mean group accuracy, indicating that frozen CLIP features are strong for average classification on this dataset. However, R4RR obtains the best worst-group accuracy, suggesting that explicitly training the classifier to allocate evidence to the target food region improves robustness on the most difficult classes. We provide qualitative examples in the Appendix~\ref{appendix:qualisamples}.


\subsection{Does R4RR improve localization?}\label{sec:pointing}

Beyond classification performance, we assess whether our training procedure improves \emph{where} the model attends, i.e., whether its explanations concentrate on task-relevant evidence. We use the \emph{Pointing Game} \cite{zhang2018top}, a standard protocol for evaluating spatial explanations. For each input image $x_i$, the Pointing Game requires (i) an explanation map $a_i \in \mathbb{R}^{H\times W}$ produced by the model, and (ii) a binary mask $Z_i \in \{0,1\}^{H\times W}$ indicating task-relevant pixels. A sample is counted as a \emph{hit} if the location of the maximum explanation value lies inside the mask. The Pointing Game score is the average hit rate over the evaluation set. Intuitively, a higher score indicates that the model's peak evidence aligns with the relevant region, suggesting that the model is ``right for the right regions'' rather than relying on spurious context. We use the bird segmentation masks in Waterbirds-95\% and Waterbirds-100\% to define $Z_i$. Figure~\ref{fig:pointing_game} shows that R4RR achieves the highest Pointing Game score in both Waterbirds settings.
...
\textcolor{red}{laterrrrrrrrrrr}
For MobileNet, we use CAM-style evidence maps from the final convolutional feature layer, exactly as in the ResNet setting. The teacher maps, KL objective, and Classify-then-Align schedule are unchanged. This experiment tests whether R4RR remains effective when the student has lower capacity and a different convolutional architecture.
\textcolor{red}{laterrrrrrrrrrr}

\subsubsection{Vision Transformer}\label{sec:ViTstudent}
\textcolor{red}{\textcolor{red}{laterrrrrrrrrrr};;;;to fix this later by Ryan}
For Vision Transformer students, CNN-style CAM is not directly available because the model operates on patch tokens rather than convolutional feature maps. We therefore construct a patch-space evidence map. Let the final transformer layer produce a CLS token $h_{\mathrm{cls}}$ and patch tokens
\begin{equation}
H_{\mathrm{patch}}(x)={h_1,\ldots,h_P}, \qquad P=H_sW_s.
\end{equation}
We compute a class-specific patch evidence score by combining final-layer CLS-to-patch attention with class-relevant patch activations. Let $\alpha_p$ denote the attention weight from the CLS token to patch token $p$, and let $g_y(h_p)$ denote the class-$y$ evidence associated with patch token $p$, obtained by projecting the patch token through the classification head or the class-specific linear weight. The patch evidence score is
\begin{equation}
e_p(x,y)=\alpha_p \cdot g_y(h_p).
\end{equation}
We then apply ReLU and reshape the resulting patch scores into the spatial patch grid:
\begin{equation}
A^{\mathrm{stu}}_\theta(x,y)=
\mathrm{Reshape}_{H_s\times W_s}\left(
\mathrm{ReLU}(e_1),\ldots,\mathrm{ReLU}(e_P)
\right).
\end{equation}
The same min--max normalization used for CNN evidence maps is then applied before distributional alignment. Thus, the R4RR loss and schedule remain unchanged for ViT students; only the evidence-map extractor differs.


\section{Ablation studies}\label{sec:ablation}

\subsection{Study on training curriculum}\label{sec:ablationtrainingcuriculum}

R4RR optimizes a classification loss $\mathcal{L}_{\mathrm{CE}}$ and an evidence-alignment loss $\mathcal{L}_{\mathrm{align}}$. Our default curriculum is \textbf{Classify$\rightarrow$Align}. We compare against two alternatives on Waterbirds: \textbf{Align$\rightarrow$Classify} and \textbf{Joint} (optimize the joint loss from scratch).
...
\begin{groupplot}[
    ...
    ymax=100,
    ylabel={Accuracy (\%)},
    symbolic x coords={AlignCls,Joint,ClsAlign},
    xtick=data,
    xticklabels={
        Align$\rightarrow$Cls,
        Joint,
        Cls$\rightarrow$Align
    },
...
\nextgroupplot[
    title={Waterbirds$_{95}$},
    legend to name=curriculumlegend
]
...
\vspace{-0.4em}
\ref{curriculumlegend}

\vspace{-0.5em}
\caption{
\textbf{Training curriculum ablation on Waterbirds.}
We compare Align$\rightarrow$Classify, joint training from scratch, and our Classify$\rightarrow$Align schedule. 
%Classify$\rightarrow$Align performs best on both Waterbirds$_{95}$ and Waterbirds$_{100}$, with the largest gain in worst-group accuracy under full bias.
}
\label{fig:curriculum_ablation}
\end{figure}


Fig~\ref{fig:curriculum_ablation} shows that the schedule strongly impacts robustness, especially in Waterbirds$_{100}$. Our \textbf{Classify$\rightarrow$Align} is best across both settings consistent with the view that a brief classification warm-start provides a stable representation that alignment can then refine away from background shortcuts.
