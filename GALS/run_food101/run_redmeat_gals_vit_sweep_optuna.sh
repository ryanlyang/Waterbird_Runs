#!/bin/bash -l
# Optuna sweep for Red Meat GALS using precomputed CLIP ViT attention maps.
#
# Tuned hyperparameters:
#   - EXP.BASE.LR
#   - EXP.CLASSIFIER.LR
#   - EXP.LOSSES.GRADIENT_OUTSIDE.WEIGHT
#   - EXP.WEIGHT_DECAY
...
#SBATCH --mem=64G
#SBATCH --output=/home/ryreu/guided_cnn/logsMeat/redmeat_gals_vit_sweep_%j.out
#SBATCH --error=/home/ryreu/guided_cnn/logsMeat/redmeat_gals_vit_sweep_%j.err
...
GRAD_WEIGHT_MIN=${GRAD_WEIGHT_MIN:-1.0}
GRAD_WEIGHT_MAX=${GRAD_WEIGHT_MAX:-100000}
...
if [[ ! -d "$DATA_ROOT/$DATA_DIR/$ATT_DIR" ]]; then
  echo "[ERROR] Missing attention maps dir: $DATA_ROOT/$DATA_DIR/$ATT_DIR" >&2
  echo "Run: sbatch run_food101/run_generate_redmeat_vit_attentions.sh" >&2
  exit 2
fi
...
echo "  grad_weight:    [$GRAD_WEIGHT_MIN, $GRAD_WEIGHT_MAX]"
...
ARGS=(--method gals
  ...
  --output-csv "$OUT_CSV"
  --logs-dir "$TRIAL_LOGS"
  --weight-min "$GRAD_WEIGHT_MIN"
  --weight-max "$GRAD_WEIGHT_MAX"
